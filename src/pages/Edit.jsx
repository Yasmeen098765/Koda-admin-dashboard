...
export default function Edit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // إضافة الصور الجديدة مباشرة (مثل QuickEdit)
      if (images.length > 0) {
      ...
      //  إرسال الطلب مع multipart/form-data
        ...
        <div className="relative p-4 mb-6 overflow-hidden transition-all duration-300 border shadow-lg sm:p-6 md:p-8 sm:mb-8 rounded-xl border-slate-200/50 dark:border-slate-700/50 bg-gradient-to-br from-white via-sky-50/80 to-blue-100/40 dark:from-slate-800 dark:via-slate-800/90 dark:to-sky-900/30 hover:shadow-xl">
          ...
          <div className="relative z-10 flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="w-full">
              <Link
                to="/products"
                className="inline-flex items-center gap-2 px-3 py-2 text-xs transition-all border rounded-lg sm:px-4 sm:text-sm border-slate-200/50 dark:border-slate-700/50 bg-white/70 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 backdrop-blur-sm"
              >
                <FiArrowLeft size={14} />
                ...
                {imagePreviews.map((preview, idx) => (
                  ...
                  >
                    ...
                    {idx >= existingImages.length && (
                      <div className="absolute top-2 sm:top-3 right-14 bg-gradient-to-r from-cyan-500 to-sky-500 backdrop-blur px-2 sm:px-3 py-1 rounded-lg text-[8px] sm:text-[10px] font-bold uppercase tracking-[0.2em] text-white shadow-sm">
                        {t("editProduct.new")}
                      </div>
                    )}
                    <button
